...
void task8()
{
	...
	printf("Here are 10 random numbers between %i and %i:",low,high);
	int i;
	for(i=1;i<=10;i++)
	{
...
void task12()
{
	...
	while(1)
	{
		goal=rand()%(10)+1;
		guess=0;
		printf("A number has been generated between 1 and 10, now guess it!\n");
		while(guess!=goal)
		{
			scanf("%i",&guess);
			if(guess==goal)
			{
				score+=5;
				printf("Correct! Current Score: %i\n\n",score);
			}
			else
			{
...
void task13()
{
	...
	for(i=1;i<=10;i++)
	{
		...
		printf("Dice roll %i: %i %i\n",i*100,dice1,dice2);
	}
	printf("\nValues and # of occurences:\n\n");
	for(i=1;i<=11;i++)
	{
		if(i>=9)
		{
			printf("%i:  %i [%3.1f%%]\n",i+1,rolls[i-1],(float)rolls[i-1]/10);
		}
		else
		{
			printf("%i:   %i [%3.1f%%]\n",i+1,rolls[i-1],(float)rolls[i-1]/10);
